...
async function start() {
  try {
    ...
    // SELF-HEALING: `members.userId`, `trainers.userId`, and
    // `attendances.userId` are logical multi-gym "scope ids" — they can
    // legitimately point at either a users.id (an owner's primary gym) or
    // a gyms.id (an additional gym). Older deployments (before multi-gym
    // support) had a real MySQL foreign key on these columns pointing
    // ONLY at `users`, which rejects every additional gym's members,
    // trainers, and attendance with "Cannot add or update a child row:
    // foreign key constraint fails". This runs on every startup and drops
    // any such constraint it finds — it only removes the constraint
    // itself, never touches the column or any row data.
    async function dropCrossScopeForeignKeys(tableName, columnName) {
      try {
        ...
        for (const row of rows) {
          try {
            ...
            console.log(`✅ ${tableName}: dropped cross-scope FK "${row.CONSTRAINT_NAME}" on ${columnName}`);
          } catch (e) {
            console.warn(`   ⚠️ could not drop FK "${row.CONSTRAINT_NAME}" on ${tableName}: ${e.message}`);
          }
        }
      } catch (e) {
        // table doesn't exist yet on a fresh database — nothing to clean, fine
      }
    }
    for (const t of ['members', 'trainers', 'attendances']) {
      await dropCrossScopeForeignKeys(t, 'userId');
    }
