      ...
      <div style="padding:12px 16px;border-bottom:1px solid #F0F5F5">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="font-size:.65rem;color:#B0C4C4;margin-top:1px">&#x1F4C5; ${s.pendingPasswordRequestedAt ? new Date(s.pendingPasswordRequestedAt).toLocaleDateString('en-IN') : ''}</div>
          </div>
        </div>
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // The service worker (Cache-First for HTML/CSS/JS, with a version string
  // that never changes between deploys) previously caused a real production
  // bug: browsers kept serving an old, mismatched mix of cached files
  // indefinitely after new deploys, even when the server had fixed files.
  // Retire it — actively unregister any existing one and wipe its caches —
  // instead of registering it again, so every page load is a normal,
  // uncached network fetch. This app doesn't rely on offline mode.
