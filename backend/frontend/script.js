              ...
              ${groupPaymentEntries(history).map(tx => `
                <div style="padding:4px 0;padding-left:12px;border-bottom:1px dashed #F0F5F5;position:relative;">
                  ...
                  <div style="position:absolute; right:0; top:50%; transform:translateY(-50%); display:flex; gap:6px;">
                    <button onclick="showReceiptOptions('${m._id}', '${esc(m.name.replace(/'/g, "\\'"))}', ${history.reduce((s,p)=>s+(p.amount||0),0)}, ${m.pendingAmount}, 'Partial Paid', '${jsEsc(tx.receiptNo || '')}')" style="background:#FFF9C4; color:#D97706; border:1px solid #FDE68A; border-radius:8px; padding:6px 10px; cursor:pointer; font-size:0.7rem;" title="Print Due Slip">📄</button>
                    <button onclick="deletePayment('${m._id}', '${tx.groupId}')" style="background:#FFF0F0; color:#E74C3C; border:1px solid #FECDD5; border-radius:8px; padding:6px 10px; cursor:pointer; font-size:0.7rem;" title="Delete this payment record">🗑️</button>
                  </div>
                </div>
              `).join('')}
${pendingBreakdown ? (() => {
