...
async function loadPwResetStatus() {
  const card = document.getElementById('pwResetCard');
  if (!card) return;
  const u = JSON.parse(localStorage.getItem('user') || '{}');
  // Superadmin has no approver above them in this flow — hide the card.
  if (u.role === 'superadmin') { card.style.display = 'none'; return; }
  card.style.display = '';

  const noteEl = document.getElementById('pwResetNote');
  if (noteEl) {
    noteEl.textContent = u.role === 'admin'
      ? 'Requesting a new password needs GymPro (superadmin) approval before it becomes active.'
      : 'Requesting a new password needs your gym admin\u2019s approval before it becomes active.';
  }
  ...
  try {
    ...
    const pending = !!me.resetPasswordRequested;
    document.getElementById('pwResetPendingBanner').style.display = pending ? '' : 'none';
    document.getElementById('pwResetFormWrap').style.display = pending ? 'none' : '';
  } catch(e) { /* ignore — form stays usable */ }
}

async function requestPasswordReset() {
  const newPass = document.getElementById('sNewPass').value;
  const confirmPass = document.getElementById('sConfirmPass').value;
  ...
  try {
    const res = await fetch(`${BASE}/auth/request-password-reset`, {
      method: 'POST', headers: hdrs(), body: JSON.stringify({ newPassword: newPass })
    });
    const data = await res.json();
    if (res.ok) {
      toast(data.message || 'Password reset requested', 'success');
      document.getElementById('sNewPass').value = '';
      document.getElementById('sConfirmPass').value = '';
      loadPwResetStatus();
    } else {
      toast(data.error || 'Failed to request reset', 'error');
    }
  } catch(e) { toast('Network error', 'error'); }
...
async function loadSuperAdminData() {
  await Promise.all([loadSaPending(), loadSaPendingGyms(), loadSaGyms(), loadSaPwResets()]);
}

async function loadSaPwResets() {
  const el = document.getElementById('saPwResetList');
  try {
    const res = await fetch(`${BASE}/auth/pending-password-resets`, { headers: hdrs() });
    if (res.status === 401) { logout(); return; }
    const list = await res.json();
    document.getElementById('saPwResetBadge').textContent = list.length;
    if (!list.length) {
    ...
    el.innerHTML = list.map(u => `
      <div style="padding:14px 16px;border-bottom:1px solid #F0F5F5">
        <div style="display:flex;align-items:center;gap:12px;margin-bottom:10px">
          <div style="width:44px;height:44px;border-radius:12px;background:linear-gradient(135deg,#6366F1,#818CF8);
          ...
          <div style="flex:1;min-width:0">
            <div style="font-weight:800;font-size:.95rem;color:#1A2E2E">${esc(u.gymName||u.name)}</div>
            <div style="font-size:.75rem;color:#4A6464;margin-top:1px">&#x1F464; ${esc(u.name)} &nbsp;|&nbsp; &#x2709;&#xFE0F; ${esc(u.email)}</div>
            <div style="font-size:.68rem;color:#8AABAB;margin-top:2px">&#x1F4C5; Requested ${new Date(u.resetPasswordRequestedAt).toLocaleDateString('en-IN')}</div>
          </div>
        </div>
        <div style="display:flex;gap:8px">
          <button onclick="saApprovePwReset('${esc(u._id)}','${esc(u.gymName||u.name)}')"
          ...
          <button onclick="saRejectPwReset('${esc(u._id)}','${esc(u.gymName||u.name)}')"
...
async function saApprovePwReset(userId, name) {
  try {
    const res = await fetch(`${BASE}/auth/approve-password-reset/${userId}`, { method:'POST', headers:hdrs() });
    const data = await res.json();
    if (res.ok) { toast(data.message || `${name}'s password reset approved`, 'success'); loadSaPwResets(); }
    else toast(data.error||'Failed','error');
  } catch(e) { toast('Network error','error'); }
}

async function saRejectPwReset(userId, name) {
  try {
    const res = await fetch(`${BASE}/auth/reject-password-reset/${userId}`, { method:'POST', headers:hdrs() });
    const data = await res.json();
    if (res.ok) { toast(`Rejected ${name}'s password reset`, 'success'); loadSaPwResets(); }
    else toast(data.error||'Failed','error');
  } catch(e) { toast('Network error','error'); }
}
...
async function loadGymAdminData() {
  await Promise.all([loadGaPending(), loadGaStaff(), loadGaPwResets()]);
}

async function loadGaPwResets() {
  const el = document.getElementById('gaPwResetList');
  try {
    const res = await fetch(`${BASE}/auth/pending-staff-password-resets`, { headers: hdrs() });
    if (!res.ok) throw new Error();
    const list = await res.json();
    document.getElementById('gaPwResetBadge').textContent = list.length;
    if (!list.length) {
      el.innerHTML = '<div class="empty"><div class="ei">&#x2705;</div><p>No pending password resets</p></div>';
      return;
    }
    el.innerHTML = list.map(s => `
      <div style="padding:12px 16px;border-bottom:1px solid #F0F5F5">
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:8px">
          <div style="width:40px;height:40px;border-radius:50%;background:linear-gradient(135deg,#6366F1,#818CF8);
          ...
          <div style="flex:1;min-width:0">
            ...
            <div style="font-size:.65rem;color:#B0C4C4;margin-top:1px">&#x1F4C5; Requested ${new Date(s.resetPasswordRequestedAt).toLocaleDateString('en-IN')}</div>
          </div>
        </div>
        <div style="display:flex;gap:7px">
          <button onclick="gaApprovePwReset('${esc(s._id)}','${esc(s.name)}')"
          ...
          <button onclick="gaRejectPwReset('${esc(s._id)}','${esc(s.name)}')"
...
async function gaApprovePwReset(userId, name) {
  try {
    const res = await fetch(`${BASE}/auth/approve-password-reset/${userId}`, { method:'POST', headers:hdrs() });
    const data = await res.json();
    if (res.ok) { toast(data.message || `${name}'s password reset approved`, 'success'); loadGaPwResets(); }
    else toast(data.error||'Failed','error');
  } catch(e) { toast('Network error','error'); }
}

async function gaRejectPwReset(userId, name) {
  try {
    const res = await fetch(`${BASE}/auth/reject-password-reset/${userId}`, { method:'POST', headers:hdrs() });
    const data = await res.json();
    if (res.ok) { toast(`Rejected ${name}'s password reset`, 'success'); loadGaPwResets(); }
    else toast(data.error||'Failed','error');
  } catch(e) { toast('Network error','error'); }
}
...
/* Shows which gym/branch is currently active — in the topbar (every page)
   and as a card at the top of the Dashboard — so an owner managing several
   gyms always knows which one they're looking at before making changes. */
function setGymBranchDisplay(u) {
  const name = u.gymName || u.name || 'Your Gym';
  const topEl = document.getElementById('topGymName');
  if (topEl) topEl.textContent = '\u{1F3E2} ' + name;
  const cardWrap = document.getElementById('dashGymBranchCard');
  const cardName = document.getElementById('dashGymBranchName');
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    if (u.role === 'superadmin') {
      ...
      if (!p.viewSettings) {
        // Keep the Settings page reachable — Reset Password lives there for
        // every role — but hide the gym-config cards a staff member without
        // this permission shouldn't see/edit.
        document.querySelectorAll('#page-settings .card').forEach(c => {
          if (c.id !== 'pwResetCard') c.style.display = 'none';
        });
      }
...
// navigator.onLine is known to false-positive as "offline" on some
// browsers/networks (e.g. flaky mobile signal, captive portals) even when
// requests actually succeed — so don't trust it alone for the banner shown
// right after load. Confirm with a real request first; only fall back to
// navigator.onLine if that check itself can't run.
(async () => {
  const banner = document.getElementById('offline-banner');
  if (!banner) return;
  if (!navigator.onLine) { banner.style.display = 'block'; return; }
  try {
    const ctrl = new AbortController();
