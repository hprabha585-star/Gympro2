...
router.post('/create-staff', verifyToken, adminOnly, async (req, res) => {
  try {
    ...
    const gymId = Number(req.user.gymId || req.user.userId); // the ACTIVE gym, not always admin.id
...
router.patch('/user/:userId/toggle', verifyToken, adminOnly, async (req, res) => {
  try {
    ...
    if (!user || user.role !== 'staff' || Number(user.gymId) !== gymId)
      return res.status(404).json({ error: 'Staff member not found for this gym.' });
