...
router.get('/gym-qr', authMiddleware, async (req, res) => {
  try {
    ...
    // userId here is the ACTIVE gym scope — could be the owner's primary
    // account (a User row) or an additional gym (a Gym row). Check both so
    // this doesn't 500 whenever an owner is switched into gym #2, #3, etc.
    let gymDisplayName = 'GymPro';
    const user = await User.findByPk(userId);
    if (user) {
      gymDisplayName = user.gymName || user.name || 'GymPro';
    } else {
      const gym = await Gym.findByPk(userId);
